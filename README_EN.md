...
<p>
  ...
  <b>Go from idea to presentation in minutes—no tedious formatting, just verbal requests for edits, moving towards real "Vibe PPT"</b>
</p>
<p>
...
- **[2026-04-25]**: Asset Toolbox launched — Added three new modes: full-image editing, marquee editing (overlay/replace), and smart erasure to the existing asset generation, providing a unified entry for one-stop operation.
...
- **[2026-04-23]**: Added support for the gpt-image-2 model. Meanwhile, the editable background effect during export has been improved due to model capability upgrades (Select "Generative Acquisition" in Settings - Export Options - Background Acquisition).
...
- **[2026-02-09]**: New Features and Optimizations
  * New Features
    * Supports pasting images on the homepage, outline, and description cards for immediate recognition, providing a better interactive experience.
...
Have you ever found yourself in this predicament: your presentation is due tomorrow, but the slides are still blank? You have countless brilliant ideas in your head, but all your enthusiasm is drained by the tedious layout and design process.

We long for a way to quickly create presentations that are both professional and well-designed. While traditional AI PPT generation apps generally meet the need for "speed," they still suffer from the following issues:
...
- 3️⃣ Similar aesthetic outcomes with severe homogenization
- 4️⃣ Low-quality assets that lack specificity
- 5️⃣ Disjointed text-and-image layout with poor design sense

These deficiencies make it difficult for traditional AI PPT generators to simultaneously satisfy our two major needs: "speed" and "aesthetics." Even those claiming to be "Vibe PPTs" fall far short of being truly "vibe" in my eyes.
...
2. **PPT Professionals**: Quickly gain design inspiration by referring to AI-generated layouts and text-image element combinations.
...
5. **Business Professionals**: Rapidly visualize business proposals and product introductions with quick adaptation for various scenarios.

<p>
  <b>🎯Goal: Lower the barrier to PPT creation, allowing everyone to quickly produce beautiful and professional presentations.</b>
</p>
...
- **Natural Language Editing**: Supports modifying outlines or descriptions through "Vibe" (e.g., "Change the third page to a case study"), with AI adjusting in real-time.
...
- **Playback Settings**: Enable slide transition animations before exporting to PPTX. Supports classic effects such as Fade, Push, Wipe, Split, Blinds, Checkerboard, and Clock, with the ability to select multiple effects for random application.
- **Perfect Fit**: Default 16:9 aspect ratio requires no secondary layout adjustments, making it ready for immediate presentation.
...
| ✅ Completed | Region selection on a single PPT slide for Vibe voice-based editing |
...
1. One-click deploy and start this application via RainYun (High bandwidth, suitable for HD image generation and downloading. New users get a 15-day free trial)
...
> **The Large Language Model (LLM) API in this project is based on the AIHubMix platform format. It is recommended to use [AIHubMix (click here to visit)](https://aihubmix.com/?aff=17EC) to obtain an API key to reduce migration costs.**<br>
...
# Provider API Keys (Only configure the ones you need to use)
...
**Use the new editable export configuration method for better editable export results**: You need to obtain an API KEY from the [Baidu AI Cloud Platform](https://console.bce.baidu.com/iam/#/iam/apikey/list) (click here to enter) and fill it in the `BAIDU_API_KEY` field in the `.env` file (ample free usage quota available). See the instructions in https://github.com/Anionex/banana-slides/issues/121 for details.
...
Note: This method is not applicable if you have manually modified the code; you must first revert the code to the version it was when pulled.
...
# Code-Assisted Development in the LLM Era

## A developer-oriented, LLM-integrated, locally-focused, and easy-to-use code indexing tool.

<p align="center">
  <a href="https://github.com/username/project/stargazers"><img src="https://img.shields.io/github/stars/username/project.svg?style=flat-square&logo=github" alt="Stars"></a>
  <a href="https://github.com/username/project/issues"><img src="https://img.shields.io/github/issues/username/project.svg?style=flat-square&logo=github" alt="Issues"></a>
  <a href="https://github.com/username/project/blob/main/LICENSE"><img src="https://img.shields.io/github/license/username/project.svg?style=flat-square" alt="License"></a>
</p>

`code-index` is a tool designed for developers to help them quickly understand, search, and manage codebases using Large Language Models (LLM). Unlike traditional grep or simple keyword searches, it provides semantic-level code understanding and retrieval capabilities.

## 🌟 Key Features

-   **Semantic Search**: Go beyond keyword matching. Understand the intent of your search and find relevant code logic.
-   **Local First**: All indexing and sensitive data stay on your local machine, ensuring codebase security.
-   **RAG (Retrieval-Augmented Generation)**: Seamlessly integrates with LLMs (like GPT-4, Claude, or local Ollama models) to answer questions based on your specific codebase.
-   **Support for Multiple Languages**: Built-in support for Python, JavaScript/TypeScript, Go, Rust, Java, and more.
-   **Extensible**: Easily integrate different Embedding models and LLM providers.
-   **Lightweight**: Low resource consumption, quick to start, and simple to configure.

## 🛠️ Usage Scenarios

-   **Understanding Legacy Projects**: Quickly grasp the architecture and logic of unfamiliar or complex legacy code.
-   **Code Review Assistant**: Use LLMs to help identify potential logic flaws or optimization opportunities.
-   **Auto-Generated Documentation**: Generate high-quality READMEs or API documentation based on indexed code.
-   **Knowledge Base Construction**: Build a private technical knowledge base for your team's code.
...
-   Git
-   Docker (Recommended) or Go environment
-   An LLM API Key (OpenAI, Anthropic, etc.) or a locally running Ollama
...
```bash
docker pull username/code-index:latest
docker run -v /path/to/your/code:/app/code username/code-index index /app/code
```
...
1.  **Initialize Configuration**:
    ...
    code-index init
    ```
    This will generate a `.code-index.yaml` file in the current directory. Edit this file to configure your LLM API and database path.

2.  **Index Your Code**:
    ```bash
    code-index index ./my-awesome-project
    ```

3.  **Perform a Semantic Search**:
    ```bash
    code-index search "How is the user authentication logic implemented?"
    ```

4.  **Ask Questions (Chat)**:
    ```bash
    code-index chat "Summarize the main workflow of the payment module."
    ```
...
│   ├── llm/            # LLM provider integration (OpenAI, Ollama, etc.)
│   └── parser/         # Multi-language code parsers (Treesitter)
...
Thank you to the following friends for their voluntary sponsorship and support:
